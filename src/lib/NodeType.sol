...
library NodeType {
    ...
    function safeCreateNode(
        ...
        assembly {
            ...
            if gt(or(_parent, or(_left, _right)), UINT32_MASK) {
    ...
    function unpack(Node node)
        internal
        pure
        returns (uint256 _value, uint256 _parent, uint256 _left, uint256 _right)
    {
        ///@solidity memory-safe-assembly
        assembly {
            _value := shr(VALUE_SHIFT, node)
            _parent := and(shr(PARENT_SHIFT, node), UINT32_MASK)
            _left := and(shr(LEFT_SHIFT, node), UINT32_MASK)
            _right := and(node, UINT32_MASK)
        }
    }
