...
contract TestPlus is Test {
    /// @dev Fills the memory with junk, for more robust testing of inline
    /// assembly
    ...
    modifier brutalizeMemory() {
        ...
        // tell the compiler that this assembly block is not in isolation.
        {
            ...
            assembly {
                let offset := mload(0x40) // Start the offset at the free memory
                    // pointer.
                ...
                // Occasionally offset the offset by a psuedorandom large
                // amount.
                ...
                for { let w := not(0) } 1 { } {
                    ...
                    // to avoid polluting the `forge test -vvvv` output with
                    // tons of junk.
    ...
    /// @dev Returns a psuedorandom random number from [0 .. 2**256 - 1]
    /// (inclusive).
    /// For usage in fuzz tests, please ensure that the function has an unnamed
    /// uint256 argument.
    /// e.g. `testSomething(uint256) public`.
    function _random() internal returns (uint256 r) {
        /// @solidity memory-safe-assembly
        assembly {
            // This is the keccak256 of a very long string I randomly mashed on
            // my keyboard.
            let sSlot :=
            ...
            // If the storage is uninitialized, initialize it to the keccak256
            // of the calldata.
            if iszero(sValue) {
                sValue := sSlot
                let m := mload(0x40)
            ...
            for { } 1 { } {
                ...
                if iszero(and(2, d)) {
                    ...
                    // Set `r` to `t` shifted left or right by a random multiple
                    // of 8.
                    switch and(8, d)
                    case 0 {
    ...
    /// Sometimes, some Solidity operations causes the free memory pointer to be
    /// misaligned.
    ...
    function _misalignFreeMemoryPointer() internal pure {
        ...
        assembly {
            ...
            m :=
                add(m, mul(and(keccak256(0x00, twoWords), 31), iszero(and(m, 31))))
            mstore(twoWords, add(m, iszero(and(m, 31))))
        }
    }

    /// @dev Check if the free memory pointer and the zero slot are not
    /// contaminated.
    ...
    {
        require(
            min <= max, "_bound(uint256,uint256,uint256): Max is less than min."
        );
    ...
    /// @dev This function will make forge's gas output display the approximate
    /// codesize of
    /// the test contract as the amount of gas burnt. Useful for quick guess
    /// checking if
    /// certain optimizations actually compiles to similar bytecode.
    function test__codesize() external view {
        /// @solidity memory-safe-assembly
        assembly {
            // If the caller is the contract itself (i.e. recursive call), burn
            // all the gas.
            if eq(caller(), address()) { invalid() }
            mstore(0x00, 0xf09ff470) // Store the function selector of
                // `test__codesize()`.
